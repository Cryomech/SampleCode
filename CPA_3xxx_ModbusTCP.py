...
def breakdownReplyData(rawData):
    ...
    wkrBytes = bytes([rawData[103], rawData[104]])
    iDeltaPressureAvg = int.from_bytes(wkrBytes, byteorder='big')
    fDeltaPressureAvg = float(iDeltaPressureAvg) / 10.0
    gTxtFeedback.insert(INSERT, "Delta Pressure Avg: " + FloatToString(fDeltaPressureAvg) + "\r\n")

    wkrBytes = bytes([rawData[105], rawData[106]])
    ...
    wkrBytes = bytes([rawData[109], rawData[110], rawData[107], rawData[108]])
    ...
    gTxtFeedback.insert(INSERT, "Software Rev: " + str(rawData[73]) + "." + str(rawData[74])  + "." + str(rawData[77])  + "\r\n")
...
def main():
    gWindow.title("CPA3.0 Format ModbusTCP")
